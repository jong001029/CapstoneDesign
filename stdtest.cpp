        ...
        if (now - lastTime >= interval) {  // 설정된 간격(10ms)이 경과했는지 확인
            ...
            if (bytesRead > 0) {  // 데이터를 읽어들였는지 확인
                ...
                if (receivedData.find("-1") != std::string::npos) {
                    ...
                    // Mask를 이용해 최종 행렬을 계산
                    std::vector<std::vector<float>> finalMatrix(height, std::vector<float>(width, 0.0f));

                    for (int i = 0; i < height; ++i) {
                        for (int j = 0; j < width; ++j) {
                            if (Mask[i][j] == 0) {
                                finalMatrix[i][j] = result[i][j];
                            } else {
                                finalMatrix[i][j] = calculateX(result, i, j, width, height);  // 이웃 값들로 x 계산
                            }
                            finalMatrixFile << finalMatrix[i][j] << " ";  // 최종 행렬 값을 파일에 기록
                        }
                    ...
                    // 업스케일링 수행
                    upscaleMatrix(finalMatrix, upscaledMatrix, width, height, newWidth, newHeight);
