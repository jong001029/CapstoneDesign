...
int main() {
    ...
    const int resamplingCount = 2; // 재샘플링 횟수 설정
            ...
            if (bytesRead > 0) {
                ...
                if (receivedData.find("-1") != std::string::npos) {
                    ...
                    for (int r = 0; r < resamplingCount; ++r) {
                        ...
                        // 현재 업스케일된 행렬을 array에 다시 넣어 재샘플링 준비
                        for (int i = 0; i < height; ++i) {
                            for (int j = 0; j < width; ++j) {
                                array[i][j] = static_cast<int>(upscaledArray[i * (newHeight / height)][j * (newWidth / width)]);
                            }
                        }
                    }

                    // 행렬을 터미널에 출력
                    std::cout << "Upscaled Matrix:" << std::endl;
                    for (int i = 0; i < newHeight; ++i) {
                        for (int j = 0; j < newWidth; ++j) {
                            std::cout << upscaledArray[i][j] << " ";
                        }
                        std::cout << std::endl;
                    }
                    std::cout << std::endl;
...
g++ -std=c++17 masktestcopy.cpp Serial.cpp -o masktest -lsfml-graphics -lsfml-window -lsfml-system
./masktest
*/